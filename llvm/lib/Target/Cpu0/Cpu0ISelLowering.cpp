...
STATISTIC(NumTailCalls, "Num of tail calls");
    ...
    if (ABI.IsO32() && IsRegLoc) {
      ...
      if (VA.getLocInfo() != CCValAssign::Full) {
        if (VA.getLocInfo() == CCValAssign::SExt) {
          OpNode = ISD::AssertSext;
        } else if (VA.getLocInfo() == CCValAssign::ZExt) {
          OpNode = ISD::AssertZext;
        }

        if (OpNode != 0) {
          ArgValue =
              DAG.getNode(OpNode, DL, RegVT, ArgValue, DAG.getValueType(ValVT));
        }
...
Cpu0TargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
  ...
  if (MF.getFunction().hasStructRetAttr()) {
    ...
    Flag = Chain.getValue(1);    // glue node: t6: ch,glue = CopyToReg t4, Register:i32 $physreg17, Constant:i32<0>
...
void Cpu0TargetLowering::Cpu0CC::handleByValArg(unsigned ValNo, MVT ValVT,
  ...
  unsigned Align =
      std::min(std::max(ArgFlags.getByValAlign(), RegSize), RegSize * 2);
  if (useRegsForByval())
    allocateRegs(ByVal, ByValSize, Align);
  // Allocate space on caller's stack.
  ByVal.Address =
      CCInfo.AllocateStack(ByValSize - RegSize * ByVal.NumRegs, Align);
  CCInfo.addLoc(
